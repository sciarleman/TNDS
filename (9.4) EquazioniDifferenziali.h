...
class OscillatoreForzante : public FunzioneVettorialeBase {

public:

  OscillatoreForzante(double wo, double alpha, double omega) { m_omega = omega; m_wo = wo; m_alpha = alpha;};

  virtual vector<double> Eval(double t, const vector<double> &x) const override {	

		vector<double> vec = { x[1], -pow(m_wo,2) *x[0] - m_alpha*x[1] + sin(m_omega*t) };
	//	Print(x);
		return vec; 
		}
