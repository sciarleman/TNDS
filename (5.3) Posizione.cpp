...
void Posizione::SetZ(double c) {

	dim_z = c;

}

void Posizione::SetY(double b) {
...
double Posizione::Distanza(const Posizione & b) const {
	return sqrt( (pow(GetComponentX() - b.GetComponentX(), 2)) + (pow(GetComponentY() - b.GetComponentY(), 2)) + (pow(GetComponentZ() - b.GetComponentZ(), 2) ));
}
